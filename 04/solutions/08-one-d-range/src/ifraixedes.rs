...
impl Range1D {
    ...
    // NOTE: This could be `&self` and the tests will continue passing. It actually was my approach,
    // but then I thought that `other` would be better not to take ownership of it, so I changed to
    // `&other`, but it some tests don't compile. If the ownership of `other` is taken, it make more
    // sense that `self` is also taken.
