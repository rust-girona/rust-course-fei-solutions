...
impl Range1D {
    fn new(start: u64, end: u64) -> Result<Range1D, String> {
        if start > end {
            Err("Start must not be larger than end".to_string())
        } else {
            Ok(Self { start, end })
        }
    }
    ...
    fn intersect(&self, range: Range1D) -> Option<Range1D> {
        let is_none = self.start > range.end || self.end < range.start;
