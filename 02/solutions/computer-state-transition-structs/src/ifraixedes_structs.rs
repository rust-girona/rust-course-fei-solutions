...
impl ComputerState {
    ...
    fn is_on(&self) -> bool {
        self.state & (STATE_RUNNING | STATE_SLEEPING) != 0
    }
...
fn pc_transition(mut computer: ComputerState, event: Event) -> ComputerState {
    match event {
        Event::TurnOn if computer.state == STATE_OFF => computer.state = STATE_RUNNING,
        Event::TurnOff => computer.turn_off(),
        Event::MoveMouse if computer.is_on() => {
            if computer.is_sleeping() {
                computer.state = STATE_RUNNING;
        ...
        Event::PassTime(t) if computer.is_on() => {
            ...
            if computer.is_sleeping() {
                ...
                if computer.idle_time() + t > 1000 {
                    let next_sleeping_time = t - (1000 - computer.idle_time);
                    // Should go to sleeping, but if it is more than 500 , then it's going off.
                    if next_sleeping_time > 500 {
                        computer.turn_off()
                    } else {
                        computer.state = STATE_SLEEPING; // OLD_BUG: I forgot this and it was a bit
                                                         // difficult to find it
                        computer.sleep_time = next_sleeping_time;
                        computer.idle_time = 0;
                    }
                } else {
        ...
        _ => { /* nothing change in whatever other transition */ }
    };
...
#[cfg(test)]
mod tests {
    use super::{pc_transition, ComputerState, Event};
