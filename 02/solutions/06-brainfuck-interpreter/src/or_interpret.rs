...
impl Block {
    ...
    fn execute(&self, exe: &mut Execution) -> Result<(), ExecuteError> {
        ...
        while let Some(instruction) = instructions.get(fp) {
            match instruction {
                ...
                Instruction::Block(block) => {
                    while exe.data[exe.dp] != 0 {
                        // debug!("Executing inner block fp: {} ni: {}", fp, exe.ni);
                        let result = block.execute(exe);
                        match result {
                            Ok(()) => (),
                            Err(err) => {
                                return Err(err);
                        ...
                        // debug!("Executed inner block fp: {} ni: {}", fp, exe.ni);
                    }
                }
            }
            fp += 1;
        ...
        /*
        debug!(
            "Executed block fp: {} dp: {} data: {} ni: {} output: {}",
            fp,
            exe.dp,
            exe.data[exe.dp],
            exe.ni,
            String::from_utf8(exe.output.clone()).expect("Failed to convert output to string")
        );
        */
        Ok(())
    }
...
impl Program {
    pub fn execute(&self, input: Vec<u8>, mut data: Vec<u8>) -> Result<String, ExecuteError> {
            ...
            match instruction {
                ...
                Instruction::Block(block) => {
                    while exe.data[exe.dp] != 0 {
                        /*
                        debug!(
                            "Executing block fp: {} dp: {} data: {} ni: {}",
                            fp, exe.dp, exe.data[exe.dp], exe.ni
                        );
                        */
                        let result = block.execute(&mut exe);
                        match result {
                            Ok(()) => (),
                            Err(err) => {
                                return Err(err);
                        ...
                        debug!(
                            ...
                            exe.data[exe.dp],
                            exe.ni,
                            String::from_utf8(exe.output.clone())
        ...
        Ok(String::from_utf8(exe.output.clone()).expect("Failed to convert output to string"))
    }
}
...
fn parse_block(chars: &mut Chars, fp: &mut usize) -> Result<Block, ParseError> {
    ...
    while let Some(c) = chars.next() {
        match c {
            ...
            '[' => {
                let result = parse_block(chars, fp);
                match result {
                    Ok(inner_block) => instructions.push(Instruction::Block(inner_block)),
