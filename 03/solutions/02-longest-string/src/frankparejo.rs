...
fn longest<'a>(first_str: &'a str, second_str: &'a str) -> &'a str {
    if second_str.len() > first_str.len() {
        return second_str;
    }
...
mod tests {
    ...
    // }
    //
    // ANSWER: It's not possible, because in case of `b` being the longest one, variable `result` will take
    // its reference. As the compiler cannot have that information at compile time, it will throw
    // an error because `result` is being used after dropping `b`.
}
