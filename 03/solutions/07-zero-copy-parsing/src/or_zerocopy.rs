...
fn is_valid_name(input: &str, required: bool) -> bool {
    let mut istr = input.chars();
    let mut valid = !required;
    while let Some(c) = istr.next() {
...
fn parse_ticket<'a>(input: &'a str) -> Option<Ticket<'a>> {
    ...
    if !is_valid_name(movie, true) {
        return None;
    }

    let day = parts.next()?;

    let day_enum = match day.to_lowercase().as_str() {
        "monday" => Day::Monday,
        "tuesday" => Day::Tuesday,
    ...
    if let Some(visitor) = parts.next() {
        if visitor.len() == 0 {
            return Some(Ticket {
                movie,
                day: day_enum,
        ...
        if !is_valid_name(visitor, false) {
            return None;
        }
        return Some(Ticket {
            movie,
