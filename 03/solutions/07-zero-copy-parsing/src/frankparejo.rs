...
impl<'a> Ticket<'a> {
    fn new(movie: &'a str, day: Day, visitor: Option<&'a str>) -> Self {
    ...
    fn validate(self) -> Option<Ticket<'a>> {
        if !Ticket::is_valid_name(self.movie) {
            return None;
        }

        if let Some(v) = self.visitor {
            if !Ticket::is_valid_name(v) {
                return None;
            }
        }
...
impl FromStr for Day {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            value => Err(format!("FromStr ERROR: invalid day {value}")),
        }
    }
}

fn parse_ticket<'a>(s: &'a str) -> Option<Ticket<'a>> {
    let mut parts = s.split(';');

    let movie = parts.next()?;
    let day = parts.next()?.parse::<Day>().ok()?;
    let visitor = parts.next();

    // It contains extra data, which is not allowed
    if let Some(_) = parts.next() {
        return None;
    }

    let ticket = Ticket::new(movie, day, visitor);
