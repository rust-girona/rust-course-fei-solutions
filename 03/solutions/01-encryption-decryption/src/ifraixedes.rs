...
impl DataEncrypted {
    pub fn read(&mut self) -> Option<u8> {
        self.original_data
            .read()
            .map(|original| original ^ self.key)
    }

    pub fn decrypt(mut self, key: u8) -> Result<Data, Data> {
        if key == self.key {
            Ok(self.original_data)
        } else {
            // When key doesn't match we must return the data, but encrypted.
            for b in self.original_data.raw_data.iter_mut() {
                *b ^= self.key;
            }

            Err(self.original_data)
        }
    }
}
