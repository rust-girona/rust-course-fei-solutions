...
impl EncryptedData {
    pub fn decrypt(self, key_candidate: u8) -> Result<Data, EncryptedData> {
        if self.encryption_key == key_candidate {
            Ok(self.unencrypted_data)
        } else {
            Err(self)
        }
    }
    ...
    pub fn read(&mut self) -> Option<u8> {
        let unencrypted = self.unencrypted_data.0.pop_front()?;
        Some(unencrypted ^ self.encryption_key)
    }
}
